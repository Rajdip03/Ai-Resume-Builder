    ...
    <div>
      ...
      <div className='flex items-center gap-2'>
        <label>
            ...
            <input type="file" onChange={(e)=>handleChange("image", e.target.files[0])} accept='image/*' className='hidden' />
        </label>
        {typeof data.image === 'object' && (
          <div className='flex flex-col gap-1 text-sm'>
            <p>Remove Background</p>
            <label className='reletive inline-flex items-center cursor-pointer text-gray-900 gap-3'>
              <input type="checkbox" className='peer sr-only' checked={removeBackground} onChange={()=>setRemoveBackground(prev=> !prev)} />
